...
var date_format = "YYYY-MM-DD HH:mm"; // chat date format
...
app._initializeWSS = function(server) {
  ...
  this._wss.on('connection', function(socket) {
    ...
    socket.initializeSession(id, room);

    // ping/pong
    socket.ping = function() {
      socket.send(JSON.stringify({ type: 'PING' }));
      socket._timeoutId = setTimeout(socket.ping, 10000);
    }
    socket._timeoutId = setTimeout(socket.ping, 10000);

    if (!id || !token || !key || !room) {
      socket.send(JSON.stringify({ type: 'ERROR', payload: { msg: 'No id, token, room or key supplied to websocket server' } }));
      socket.close();
      return;
    }
    
    if (!self._clients[key] || !self._clients[key][id]) {
      self._checkKey(key, ip, room, function(err) {
        if (!err) {
          if (!self._clients[key][id]) {
            self._clients[key][id] = { token: token, ip: ip, room: room };
            self._ips[ip]++;
            self._room_members[room][id] = id;
            
            socket.send(JSON.stringify({ type: 'OPEN' }));
          }
          self._configureWS(socket, key, id, token, room);
        } else {
...
app._configureWS = function(socket, key, id, token, room) {
  ...
  socket.on('close', function() {
    // remove from functions of timeout
  ...
  socket.on('message', function(data) {
    try {
      ...
      if (['LEAVE', 'CANDIDATE', 'OFFER', 'ANSWER'].indexOf(message.type) !== -1) {
        ...
        // attach date
        message.payload.date = moment().tz('Asia/Tokyo').format(date_format);
        
        self._sendToRoom(key, message.type, id, room, message.payload);
      } else {
...
app._checkKey = function(key, ip, room, cb) {
  if (key == this._options.key) {
    ...
    if (!this._room_members[room]) {
      this._room_members[room] = {};
    }
...
app._initializeHTTP = function() {
  ...
  this.post('/:key/:id/:token/:room/id', function(req, res, next) {
    ...
    util.extend(this, session);
    this.initializeSession(id, room);
  ...
  // Get a list of room member peers for a key.
  this.get('/:key/:room/room-peers', function(req, res, next) {
    var room = req.params.room;
    if (self._room_members[room]) {
      res.send(Object.keys(self._room_members[room]));
    } else {
      res.sendStatus(404);
    }
  });
...
app._removePeer = function(key, id) {
  if (this._clients[key] && this._clients[key][id]) {
    ...
    // send to members
    this._sendToRoom(key, "LEAVE", id, room);
...
app._sendToRoom = function (key, type, src, room, payload) {
  payload = payload || {};
  ...
  for (var k = 0, kk = members.length; k < kk; k += 1) {
    ...
    if (member != src) {
      ...
      this._handleTransmission(key, message);
    }
  }
}
